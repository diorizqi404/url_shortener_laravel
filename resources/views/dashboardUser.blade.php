...
<body class="flex justify-center items-center flex-col" data-is-new-user="{{ auth()->user()->is_new ? '1' : '0' }}">
    <style>
        * {
    ...
    <section class="w-[95%] xl:w-1/2 my-16">
        <div class="flex justify-between mb-4 flex-wrap">
            ...
            <div class="flex">
                <button id="start-tour"
                    class="mr-4 py-2 px-3 rounded-lg border border-yellow-500 hover:bg-yellow-500 hover:text-white transition-all duration-200 ease-in-out">
                  🛫 Need Help?
                </button>
                <button onclick="showCPModal()"
                    class="mr-4 py-2 px-3 rounded-lg border border-blue-500 hover:bg-blue-500 hover:text-white transition-all duration-200 ease-in-out">
                    🧑‍💻Profile
                </button>
        ...
        {{-- NOTIFICATION --}}
        @if (session('success'))
            <div class="success_notif bg-green-500 text-white p-4 rounded-lg mb-4" id="notif">
        ...
        <form action="{{ route('generate-url') }}" method="POST" class="border-2 border-slate-300 p-4 my-4 rounded-xl">
            ...
            <br><br>
            <label for="shortened_url" class="font-semibold text-lg">🖊️Custom Your URL</label>
						<br>
            <label for="shortened_url" class="text-red-500 tracking-tighter text-[0.8rem]">⚠️Isi custom url kamu, Klik
                tombol "Random" untuk generate url secara acak.</label>
								<br>
            <label for="shortened_url" class="font-semibold text-lg">{{ $domain . '/' }}</label>
            <input type="text" name="shortened_url" class="custom_url border border-gray-400 rounded pl-2 h-10 w-1/2"
                placeholder="Enter Custom URL" id="custom-url-create">
            <button id="btn-random-url" type="button" name="generate"
            ...
            <br>
            <button id="btn-create-url" type="submit" name="submit" onclick="saveTourState()"
                ...
                <div class="border-b-2 border-slate-300 p-2 w-full mb-2 flex justify-between flex-col flex-wrap">
                    <div class="mb-4">
                        <div id="shortened_url">
                            ...
                            ><span class="text-blue-500">{{ $domain . '/' . $u->shortened_url }}</a>
												</div>
                        {{-- info bawah --}}
                        <div class="mt-2 flex flex-wrap justify-between">
                            <h1 class="mr-4">🔗{{ \Illuminate\Support\Str::limit($u->original_url, 50) }}</h1>
                            <div class="flex">
                                <h1 id="url-created-at" class="mr-4">🕑{{ $u->created_at }}</h1>
                                <h1 id="url-clicks" class="mr-4">👆{{ $u->clicks }}</h1>
                            </div>
                        </div>
                    </div>
                    <div class="xl:mt-0">
                        <button id="btn-copy-url" onclick="copyLink('link{{ $loop->iteration }}')"
                        ...
                        <button id="btn-delete-url" onclick="showDeleteModal('{{ $u->shortened_url }}', {{ $u->id }})"
                            class="py-1 px-3 rounded-lg border border-blue-500 hover:bg-blue-500 hover:text-white transition-all duration-200 ease-in-out">
                            🗑️Delete
                        </button>
                    </div>
    ...
    <div class="z-50 absolute flex justify-center items-center w-full">
        <form action="{{ route('edit-url', ['id' => request()->input('id')]) }}" method="POST"
            ...
            <input type="text" name="shortened_url" class="border border-gray-400 rounded pl-2 h-10 w-1/2"
                placeholder="Enter Custom URL" id="custom-url-edit">
    ...
    {{-- PROFILE MODAL --}}
    <div class="z-50 absolute flex justify-center items-center w-full">
        <form action="{{ route('change-password') }}" method="POST" id="cpModal"
            class="flex items-start flex-col p-4 w-1/4 border border-2 rounded-xl bg-white hidden">
            @csrf
            @method('PUT')
            ...
            @enderror
            <button onclick="showCPModal()"
                class="text-center mt-8 py-2 px-3 rounded-lg border border-blue-500 hover:bg-blue-500 hover:text-white transition-all duration-200 ease-in-out">
                🔒Change Password
            </button>
            <button onclick="deleteAllForm()"
                class="text-center mt-4 py-2 px-3 rounded-lg border border-blue-500 hover:bg-blue-500 hover:text-white transition-all duration-200 ease-in-out cursor-pointer">🔙Cancel</button>
        </form>
    </div>
    ...
    <script>
			document.body.dataset.isNewUser = @json(auth()->user()->is_new);
