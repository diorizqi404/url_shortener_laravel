...
<body class="flex justify-center items-center flex-col">
    ...
    <section class="w-[95%] xl:w-1/2 my-16">
        <div class="flex justify-between">
            ...
            <form id="logout-form" action="{{ route('logout') }}" method="POST" class="d-none">
                @csrf
                <button type="submit"
                    class="py-2 px-3 rounded-lg border border-red-500 hover:bg-red-500 hover:text-white transition-all duration-200 ease-in-out">🔓Logout</button>
            </form>
        </div>
        ...
        <form action="{{ route('generate-url') }}" method="POST" class="border-2 border-slate-300 p-4 my-4 rounded-xl">
            ...
            <button type="button" name="generate"
                ...
                onclick="generateRandomURL()">🔀Random</button><br>
            <button type="submit" name="submit"
                class="mt-8 py-2 px-3 rounded-lg border border-blue-500 hover:bg-blue-500 hover:text-white transition-all duration-200 ease-in-out">📥Create
                Shorten URL</button>
        </form>
                ...
                <div class="border-b-2 border-slate-300 p-2 w-full mb-2 flex justify-between flex-col flex-wrap">
                    ...
                    <div class="xl:mt-0">
                        <button onclick="copyLink('link{{ $loop->iteration }}')"
                            class="h-auto py-1 px-3 rounded-lg border border-blue-500 hover:bg-blue-500 hover:text-white transition-all duration-200 ease-in-out">📋Copy</button>
                        <button
                            onclick="showEditForm('{{ $u->original_url }}', '{{ $u->shortened_url }}', {{ $u->id }})"
                            class="py-1 px-3 rounded-lg border border-blue-500 hover:bg-blue-500 hover:text-white transition-all duration-200 ease-in-out">✏️Edit</button>
                        <button onclick="showDeleteModal('{{ $u->shortened_url }}', {{ $u->id }})"
                            class="py-1 px-3 rounded-lg border border-blue-500 hover:bg-blue-500 hover:text-white transition-all duration-200 ease-in-out">🗑️Delete</button>
                    </div>
                </div>
            @endforeach
        </div>
    ...
    <div class="z-50 absolute flex justify-center items-center w-full">
        <form action="{{ route('edit-url', ['id' => request()->input('id')]) }}" method="POST"
            ...
            <button type="button" name="generate"
                ...
                onclick="generateRandomURL()">🔀Random</button><br>
            <button type="submit" name="submit"
                class="mt-8 py-2 px-3 rounded-lg border border-blue-500 hover:bg-blue-500 hover:text-white transition-all duration-200 ease-in-out">📤Edit
                URL</button>
            <a onclick="hideEditForm()"
                class="mt-8 py-2 px-3 rounded-lg border border-blue-500 hover:bg-blue-500 hover:text-white transition-all duration-200 ease-in-out">🔙Cancel</a>
    ...
    <div class="z-50 absolute flex justify-center items-center w-full">
        <form action="{{ route('delete-url', ['id' => request()->input('id')]) }}" method="POST"
            ...
            <div class="flex justify-between">
                ...
                <button type="submit" name="submit"
                    class="mt-8 py-2 px-3 rounded-lg border border-red-500 hover:bg-red-500 hover:text-white transition-all duration-200 ease-in-out">🗑️Delete</button>
            </div>
            <input name="id" type="hidden" value="">
        </form>
    </div>
    ...
    {{-- PROFILE MODAL --}}
    {{-- <div class="flex justify-center items-center w-full" id="profileModal">
			<p class="text-9xl"></p>
			<h1>username</h1>
			<button onclick="generateRandomEmoji()">random emoji</button>
    </div> --}}
    ...
    @endif
    <form action="{{ route('change-password') }}" method="POST" class="mb-16">
        @csrf
        @method('PUT')
    ...
    <script>
        ...
        let hideDeleteModal = (shortened_url, id) => {
            let deleteModal = document.getElementById('deleteModal')
            let overlay = document.getElementById('overlay')
